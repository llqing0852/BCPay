    ...
    public partial class Main : Form
    {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridView1.Columns[e.ColumnIndex].Name == "btnDelete" && e.RowIndex >= 0)
            {
                ...
                {
                    DataAccess.ExecuteNonQuery($"delete from wechat_accounts where domain_name='{domainName}'");
                    ReloadDatatable1();
                }
            }
        }
        ...
        private void ReloadDatatable1()
        {
            ...
            var dt = DataAccess.ExcuteDataTableReader("select today_amount,yesterday_amount,(total_amt-today_amount) remain_amount, domain_name,comments,settle_status from wechat_accounts order by domain_name");
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (currentDomainName != domainName || currentComments != comments)
                {
                    if (currentDomainName != null && currentComments != null)
                    {
                        ...
                        //newRow["pass_rate"] = GetPassRateByDomainName(currentDomainName);
            ...
            //newRow2["pass_rate"] = GetPassRateByDomainName(currentDomainName);
        ...
        private string GetPassRateByDomainName(string domainName)
        {
            var today = DateTime.Today.ToString("yyyy-MM-dd");
            int passCount = 10;
            bool hasAccounts = false;
            bool hasTrade = false;
            DataAccess.ExecuteReader($"select account from wechat_accounts where domain_name='{domainName}' and settle_status=1", reader =>
            {
                while (reader.Read())
                {
                    hasAccounts = true;
                    ...
                    int n = 0;
                    DataAccess.ExecuteReader($"select trade_state from tb_pay_log where ali_account='{account}' and create_time>'{today}' and total_fee>100 order by create_time desc limit 0,10", reader2 =>
                    {
                        while (reader2.Read())
                        {
                            hasTrade = true;
                        ...
                        if (hasTrade && n < 10 && n > 0)
                        {
                            count = (count * 10) / n;
                        }
                    });

                    if (count < passCount) passCount = count;
                }
            });

            return hasAccounts ? (hasTrade ? (passCount * 10) + "%" : "未开始交易") : "无开启通道";
        }
        ...
        {
            var today = DateTime.Today.ToString("yyyy-MM-dd");
            var dt = Db.DataAccess.ExcuteDataTableReader($"select count(*) cnt, mall_name,card_owner from trade_order where status=0 and pay_type='Wap' and create_time>'{today} 00:00:00' group by mall_name,card_owner order by cnt desc limit 0,15");
        ...
        private void dataGridView3_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView3.Columns[e.ColumnIndex].Name == "btnCheck" && e.RowIndex >= 0)
            {
                ...
                var client = new RestClient("http://156.237.190.247/shopx5/pay/createWxOrder");
        ...
        private void RefreshTodayToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            DataAccess.ExecuteReader("select account from wechat_accounts", reader =>
            {
                while (reader.Read())
                {
                    ...
                    if (todayDic.ContainsKey(account))
                    {
                        todayAmt = todayDic[account];
                    }

                    if (yesterdayDic.ContainsKey(account))
                    {
                        yestodayAmt = yesterdayDic[account];
                    }

                    DataAccess.ExecuteNonQuery($"update wechat_accounts set today_amount={todayAmt},yesterday_amount={yestodayAmt} where account='{account}'");
                }
            });
